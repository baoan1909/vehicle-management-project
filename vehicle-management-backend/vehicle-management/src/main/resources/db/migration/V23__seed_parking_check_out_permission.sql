...
VALUES ('CHECK_OUT', 'Check out', 'Execute parking check-out command.')
ON CONFLICT (code) DO NOTHING;

INSERT INTO iam.permissions (permission_code, module_id, action_id, scope_id, name, description)
SELECT 'PARKING_SESSION_CHECK_OUT_ALL',
       m.module_id,
       a.action_id,
       s.scope_id,
       'Check out parking session',
       'Allow executing parking session check-out workflow.'
FROM iam.permission_modules m
         JOIN iam.permission_actions a ON a.code = 'CHECK_OUT'
...
FROM iam.roles r
         JOIN iam.permissions p ON p.permission_code = 'PARKING_SESSION_CHECK_OUT_ALL'
