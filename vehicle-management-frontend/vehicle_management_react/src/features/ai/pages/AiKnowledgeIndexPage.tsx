...
import { Badge, Button, Input, Modal, SelectMenu } from "@/components/ui";
import {
  ...
  return (
    <main className="tw-grid tw-gap-4 tw-p-5 max-[700px]:tw-p-3">
      <section className="tw-flex tw-flex-wrap tw-items-center tw-justify-between tw-gap-3">
        <div>
          <h1 className="tw-m-0 tw-text-xl tw-font-black tw-text-slate-900">Quản lý chỉ mục tri thức</h1>
          <p className="tw-m-0 tw-mt-1 tw-text-sm tw-font-semibold tw-text-slate-500">Vòng đời phiên bản chỉ mục kiến thức, embedding và kích hoạt tìm kiếm AI.</p>
        </div>
        {canCreate ? (
